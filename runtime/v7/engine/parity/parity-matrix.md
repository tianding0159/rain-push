...
| R-DARK-01 | 5 | emotion/behavior | high | partial (bidirectional-gate-missing) | tested | Sensitive input must pass a BIDIRECTIONAL gate by provenance: NEGATIVE (keyword-only dark/sexual/drug must not auto-activate crisis/intimacy/severe) AND POSITIVE (a confirmed_current_event / confirmed_harm_evidence / canon_route must be ABLE to activate a genuinely severe state), plus an operational-guidance language gate. Fix = add the missing POSITIVE paths, NOT a blanket block. |
...
- **Expected packet field:** `meaning.data.meanings + meaning.data.blockedMeanings + emotion.data.blocked + behavior.data.actionType`  
- **Requirement:** Sensitive input must pass a **bidirectional** gate with distinct paths by *provenance*. **NEGATIVE direction (keyword-only):** a bare dark / sexual / drug reference must not auto-*activate* a severe state (crisis / intimacy / severe). **POSITIVE direction (real events):** a `confirmed_current_event`, `confirmed_harm_evidence`, or a `canon_route` must be **able** to activate a genuinely severe state — the gate must not flatten real emergencies or Canon-scripted severe events into the same non-engagement as a keyword joke. Separately, operational-guidance phrasing must be stripped at the **Language/Safety layer**. The correct fix is **adding the missing POSITIVE activation paths, not a blanket restraint block** (a blanket block would suppress exactly these real events).  
- **Gates:** `keyword_only_negative_gate`: partial · `confirmed_current_event_positive_gate`: missing · `confirmed_harm_positive_gate`: missing · `canon_route_positive_gate`: missing · `operational_guidance_language_gate`: missing · `harmless_fallthrough`: implemented  
- **Implementation (partial, bidirectional-gate-missing):** NEGATIVE direction (partial): meaning.js gives each of dark_humor / sexual_joke / drug_reference a negative-inference meaning (`dark_language_context_not_automatically_crisis` / `sexual_joke_not_automatically_intimacy` / `drug_reference_not_automatically_severe`); dark & sexual additionally add blockedMeanings (`automatic_crisis` / `automatic_intimacy`), drug does not. POSITIVE direction (MISSING): there is NO activation path in any runtime that lets a `confirmed_current_event`, `confirmed_harm_evidence`, or a `canon_route` re-enable a real severe state — a genuine emergency or a Canon-scripted severe event is flattened to the same harmless non_engagement as a keyword joke. `operational_guidance_language_gate` (MISSING): no Language/Safety-layer gate strips operational-guidance phrasing. `harmless_fallthrough` (implemented): with no positive path, everything falls through to non_engagement / no rendered text. The gap is the MISSING positive direction (three activation gates + operational-guidance gate), not "add a drug block". _(src/runtimes/meaning.js, emotion.js, decision.js, behavior.js)_  
- **Validator:** meaning.js emits negative-inference meanings for all three and blockedMeanings for dark & sexual (partial negative gate); there is NO validator or runtime path implementing the positive activation gates (confirmed current event / confirmed harm / canon route) or the operational-guidance language gate.  
- **Test (tested):** bidirectional gate — negative direction locked for dark & sexual; each positive activation path (confirmed_current_event / confirmed_harm / canon_route) + the operational-guidance gate pinned as still-missing via temporary pins _(tests/parity-contract.test.js + tests/property-conformance.test.js)_
